    ...
    computed: {
      ...
      allRoles() {
        ...
        if (this.resourceType === 'Repository') {
          return ['reader', 'admin', 'maintainer', 'editor'].filter((role) =>
      ...
      resourceIds() {
        if (this.resourceType === 'Organization') {
          return (
            this.state.currentLevel?.organizations ?? this.state.organizations
          );
        }
        if (this.resourceType === 'Repository') {
          return (
            this.state.currentLevel?.repositories ?? this.state.repositories
          );
        }
    ...
    methods: {
      async sendRoleFact() {
        if (
          !this.userId ||
          !this.role ||
          ((!this.resourceType || !this.resourceId) && !this.isGlobalRole)
        ) {
